...
class XUIClient {
  ...
  async login(): Promise<void> {
    try {
      ...
      this.sessionCookie = setCookie.map((c: string) => c.split(';')[0]).join('; ');
    } catch (error: any) {
  ...
  async createClient(inboundId: number, clientData: ClientData): Promise<ApiResponse> {
  ...
  async deleteClient(inboundId: number, clientId: string, email: string): Promise<ApiResponse> {
    await this.ensureAuth();

    try {
      const response = await axios.post(
        `${this.baseURL}/panel/api/inbounds/delClient`,
        {
          id: inboundId,
  ...
  async deleteClientAlternative(
    inboundId: number,
    criteria: { clientId?: string; email?: string; subId?: string; tgId?: string }
  ): Promise<ApiResponse> {
    await this.ensureAuth();
    ...
    try {
      ...
      const updateResponse = await axios.post(
        `${this.baseURL}/panel/api/inbounds/update/${inboundId}`,
        updateData,
        {
          headers: {
            'Cookie': this.sessionCookie,
